...
namespace FS.Editor
{
    public class TableView<TData>
    {
        ...
        public void DrawTableGUI(TData[] data, float maxHeight = float.MaxValue, float rowHeight = -1)
        {
            ...
            int k_seperatorPadding = 10;

            for (int row = 0; row < data.Length; row++)
            {
                Rect rowRect = new Rect(0, (rowHeight + k_seperatorPadding) * row, rowWidth, rowHeight);
                ...
                float k_seperatorHeight = 2.5f;
                var seperatorRect = new Rect(0, rowRect.yMax + (k_seperatorPadding - k_seperatorHeight)/2f, rowWidth, k_seperatorHeight);
                EditorGUI.DrawRect(seperatorRect, Color.black);
            }
