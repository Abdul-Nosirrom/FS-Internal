...
public class SpeedLines : MonoBehaviour, ICommandBufferPass
{
    ...
    private void GenerateCylinderMesh()
    {
        ...
        for (int i = 0; i <= m_numSegments; i++)
        {
            ...
            // Computer vertex positions in WS and set ModelMat to identity
            var right = transform.right;
            var up = transform.up;
            var forward = transform.forward;

            vertices[i * 2] = targetPos + (right * x + up * z + forward) * 2;
            vertices[i * 2 + 1] =  transform.position + (right * x + up * z) * 4;
            
            vertices[i * 2] = new Vector3(2*x, 2*z, 4); // Top vertex
            vertices[i * 2 + 1] = new Vector3(3*x, 3*z, -4); // Bottom vertex
    ...
    public void OnCameraRender(CommandBuffer cmd)
    {
        ...
        // TODO: Utilize PlayerRenderData
        cmd.SetGlobalFloat(k_playerSpeedID, speed);

        if (m_smoothedVelocity.IsNearlyZero(1f)) return;

        var targetForward = m_smoothedVelocity;
        //Matrix4x4 matrix = Matrix4x4.TRS(targetPos - m_target.up, Quaternion.LookRotation(targetForward, Vector3.up), new Vector3(1, 1, 1) * m_cylinderSize);
        Matrix4x4 matrix = Matrix4x4.TRS(transform.position, Quaternion.LookRotation(targetForward, Vector3.up), new Vector3(1, 1, 1) * m_cylinderSize);
