...
[AddComponentMenu("Free Skies/Camera/Cylindrical Camera")]
public class VCamMode_Cylinder : VirtualCameraMode
{
    ...
    public Vector3 CamForward => -(m_cameraOwner.transform.position - transform.position).ProjectOnPlane(CamUp).normalized;
    public Vector3 CamRight => -Vector3.Cross(CamForward, Vector3.up).normalized;
    ...
    protected override void OnCameraActivated()
    {
        ...
        if (lockMovement && m_physics)
        {
            ...
            {
                var planeAxis = CamForward.ProjectOnPlane(Vector3.up);
    ...
    public override void UpdateCamera()
    {
        ...
        cameraState.m_position = m_currentFollowTarget - CamForward * Distance;
        cameraState.m_rotation = Quaternion.Slerp(cameraState.m_rotation,
            Quaternion.LookRotation(CamForward, Vector3.up) * RotationOffset.ToQuaternion(), 10f * Time.deltaTime);
